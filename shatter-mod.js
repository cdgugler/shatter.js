...
async function loadImage(elem, url) {
...
function Shatter (img, numPolys, scale) {
    ...
    this.polygons = this.getPolys(img.width, img.height, numPolys);
    this.roundVertices(this.polygons);
    this.calcBoundaries(this.polygons, this.img);
    this.scaleCoordinates(this.polygons, scale);
    this.setImages();
};

Shatter.prototype.setImages = async function() {
    this.images = await this.spliceImage(this.polygons, this.img);
};
 ...
 * 
 */
Shatter.prototype.spliceImage = async function (polygons, img) {
    imageList = [];
    ...
    polygons.forEach(async function (polygon) {
        // Draw clipping path for the current polygon on the 2d context
        var tempBigImage = await Shatter.prototype.getClippedImage(polygon, tempCtx, tempCanvas, img);
        var croppedImage = await Shatter.prototype.getCroppedImage(polygon, tempBigImage);
        console.log('Got both temp and cropped');
 ...
 */
Shatter.prototype.getClippedImage = async function(polygon, ctx, tempCanvas, img) {
    ...
    let res = await loadImage(tempBigImage, tempCanvas.toDataURL("image/png"));
    console.log('Loaded images in getClippedImage');
    return res;
 ...
 */
Shatter.prototype.getCroppedImage = async function (polygon, tempBigImage) {
        ...
        let res = await loadImage(saveImage, cropCanvas.toDataURL("image/png"))
        console.log('Loaded in Cropped images');
        cropCanvas = null;
