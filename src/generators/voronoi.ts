...
export default function VoronoiPieces(
    { height = 100, width = 100, numPieces = 4 }: { height: number; width: number; numPieces: number }
): Piece[] {
    const vertices = Array(numPieces).fill(0).map(() => {
    ...
    return pieces.map(piece => {
        return piece.filter(point => point !== piece['data']).map(point => {
            return [
                Math.ceil(point[0]),
                Math.ceil(point[1])
            ];
